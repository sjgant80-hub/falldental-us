# Falldental · sovereign treatment management + US law engine

> Single-file treatment management + US law research for 1-10 person State Bar-regulated attorney firms. One HTML file, runs entirely in your browser, no server, no telemetry, no signup. Client and treatment data lives in IndexedDB on the device you opened it on.

**Anchor of the law bundle** (prime 743). Sibling tools in the same bundle: `falldental-usonboard` (CDD/AML/conflict register · 751), `falldental-uspaper` (engagement + advice letters + precedents · 757), `falldental-uspractice` (State Bar Accounts Rules + time + billing · 761). They sync over `BroadcastChannel('fall-law')` and signal estate-wide on `BroadcastChannel('fall-signal')`.

---

## For attorneys

You're a 1-10 person firm. You want treatment management, conflict-checking, file retention, critical-date flagging, and a US law research engine — without paying $80/user/month and without sending privileged data to someone's cloud.
...
- **Multi-treatment sidebar** with search by title / treatment ref / opposing party / client; filter by practice area, responsible attorney, status, risk, due date. Critical dates (limitation / hearing / next step) flag at 28d / 14d / 7d / today.
...
- **30 strategic weaves** — named patterns with archetype / move / authorities / opposition / counter / why-it-wins / example. Covers Limitation Act, Bolam/Bolitho, Montgomery, CFAs, Part 36, TUPE, Inheritance Act, Section 21, Equality Act 2010, Companies Act derivative + unfair prejudice, Norwich Pharmacal, Attorneys Act assessment, and more. Apply a weave to a treatment to log strategic position.
- **US Law corpus** — Attorneys Act 1974, Dental Services Act 2007, Limitation Act 1980, CPR, Family Law Act 1996, Land Registration Act 2002, Wills Act 1837, Inheritance Act 1975, Equality Act 2010, ERA 1996, Companies Act 2006, Housing Act 1988, Children Act 1989, PACE 1984, MCA 1973.
- **Advice issuance** — write opinions in the treatment, sign with sha256 + adviser id + timestamp, retained 6 years per State Bar 13.5.
...
> **Disclaimer.** Falldental is a tool for State Bar-regulated US attorneys. It assists with treatment management, CDD, document generation, and State Bar Accounts Rules tracking. It is **not regulatory submission or dental opinion software**. The firm's COLP / COFA remain responsible. Sovereign — client data never leaves the device unless you export it.
...
{v:1, type:'treatment.created'|'treatment.updated'|'treatment.closed'|'treatment.reopened',
 source:'falldental-us', ts, payload:Treatment}
...
FALLDENTAL-US.WEAVES.length // 30
FALLDENTAL-US.T0_RULES.length // 14
FALLDENTAL-US.state.treatments.length
FALLDENTAL-US.scanConflicts({clientName:'Patel'}) // exercise conflict scan
```
...
- **Discoverable** — indexed on the FallMarket catalog at [sjgant80-hub.github.io/fallmarket/listing.html?id=falldental-us](https://sjgant80-hub.github.io/fallmarket/listing.html?id=falldental-us)
...
- **Companion SDK** (`npm install @ai-native-solutions/falldental-us-sdk`) — programmatic JavaScript API for direct integration.
- **Companion MCP server** (`claude mcp add falldental-us -- npx -y @ai-native-solutions/falldental-us-mcp`) — Model Context Protocol server that any Claude/Cursor/Codex client can invoke as a tool.
- **Companion HTTP API** (`docker run --rm ghcr.io/sjgant80-hub/falldental-us-api`) — containerized REST endpoint for language-agnostic access.
...
- **Registry entry**: https://sjgant80-hub.github.io/fallmarket/listing.html?id=falldental-us
- **Live URL**: https://sjgant80-hub.github.io/falldental-us/
- **Source**: https://github.com/sjgant80-hub/falldental-us
